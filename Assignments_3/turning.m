...
for i= 1:t_final
    ...
    f(i) = 2.032;  %N/s (the value assumed as same at the cruise consumption)
    D(i) = (0.5).*ro(i).*(x4(i)^2).*S.*cD(i);
    
    if x5(i) > 14
    u1(i)= D(i)+(x6(i).*g.*sind(path_angle(i)));
    der_x1= x4(i).*cosd(x5(i)).*cosd(path_angle(i));
    der_x2= x4(i).*sind(x5(i)).*cosd(path_angle(i));
    der_x3= x4(i).*sind(path_angle(i));
    der_x4= -((0.1136*S.*ro(i)*((x4(i)).^2))/(2*x6(i))) - (g.*sind(path_angle(i))) + ((u1(i))/(x6(i)));
