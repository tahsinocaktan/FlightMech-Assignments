%**************** TAHSİN OCAKTAN ***************%
%%%%%%%%%%%%%%% FLIGHT MECHANICS %%%%%%%%%%%%%%%%
%%%%%%%%%% TERM HOMEWORK-TAKE OFF PHASE %%%%%%%%%

clc;
clear;

%*********adjust constant variable 
h = 0; % altitude in meter
S_G_sl = 3896; % take off ground roll at sea level
g = 9.80665; % gravitational acceleration
S = 511; % wing area
c = 0.93; % 1/h
m_r = 0.025; % friction coeff
k = 0.043; 
%---------------------------------------------------
...
for i= 1:t_final
     ...
     cL(i) = m_r/(2*k); % lift force constant value 
     cD(i) = 0.028 +  k*(cL(i).^2); % drag force constant value
     D(i) = (0.5).*ro(i).*(x4(i)^2).*S.*cD(i); % drag force
     L(i) = (0.5).*ro(i).*(x4(i)^2).*S.*cL(i); % lift force

     if x1(i) <= S_G_sl

     u1(i+1)= D(i)+ m_r*(x6(i)*g - L(i)); % thrust
     f(i) = c.*u1(i)./3600;  %N/s
